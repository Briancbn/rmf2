...
class AmqpExecutorTransport(ExecutorBaseTransport):
    ...
    def start(self) -> None:
        ...
        self._ioloop_thread = threading.Thread(target=self._run, daemon=True, name="amqp-executor-ioloop")
    ...
    def _on_exchange_declared(self, _frame) -> None:
        logger.info("AMQP executor transport connected — exchange '%s'", self._exchange)
        with self._pending_lock:
